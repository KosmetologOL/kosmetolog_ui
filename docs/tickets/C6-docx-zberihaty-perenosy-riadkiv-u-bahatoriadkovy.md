...
- [ ] Дворядковий DEFAULT_FINAL_NOTE у дописаному .docx відображається двома рядками (телефон на новому рядку), як у HTML-версії.
- [ ] Багаторядкові «Все, що необхідно знати про ваш стан», «Додаткова інформація» і коментарі до процедур зберігають переноси рядків у Word.
- [ ] Однорядкові значення цих полів рендеряться без змін (без зайвих розривів).
