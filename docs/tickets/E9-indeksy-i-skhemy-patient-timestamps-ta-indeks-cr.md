...
- [x] db.patients.getIndexes() містить { createdAt: -1 }; db.specialists і db.homecares — unique-індекс по name; db.categoryitems — унікальний компаундний { category: 1, name: 1 }.
- [x] Створення спеціаліста/догляду з наявною назвою повертає помилку, а не другий запис.
- [x] Нові пацієнти отримують createdAt і updatedAt автоматично; список пацієнтів працює як раніше (порядок за датою створення, lastVisitAt на місці).
- [x] Жодного автоматичного видалення чи модифікації наявних дублікатів — лише список для власника.
