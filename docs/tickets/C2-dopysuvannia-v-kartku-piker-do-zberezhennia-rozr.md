...
- [ ] Пікер файлу відкривається одразу після кліку «Додати в картку (.docx)», ДО збереження звіту — навіть якщо збереження триває понад 5 секунд, пікер не зникає мовчки.
- [ ] Скасування пікера (Escape) не показує жодного повідомлення; збій відкриття пікера і відмова в дозволі показують різні українські тости.
- [ ] Якщо назва вибраного файлу не містить прізвища пацієнта — зʼявляється підтвердження з назвою файлу та ПІБ пацієнта; відмова скасовує операцію без запису.
- [ ] Перед кожним перезаписом картки у «Завантаження» падає копія попередньої версії з розширенням .bak, і тост успіху про це повідомляє.
- [ ] У непідтримуваному браузері (Firefox/Safari) спроба дописування показує тост «…доступне лише в Chrome або Edge», як і раніше.
