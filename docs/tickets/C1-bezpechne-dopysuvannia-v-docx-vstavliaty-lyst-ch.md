...
- [x] insertParagraphsBeforeFinalSectPr не містить рядкового пошуку <w:sectPr — точка вставки визначається через DOM як пряма дитина w:body.
...
- [x] Некоректний (необроблюваний парсером) document.xml призводить до тосту про помилку структури, а файл на диску лишається байт-у-байт незмінним.
- [x] Результуючий document.xml починається з оригінальної XML-декларації <?xml …?>.
