...
- [ ] GET /reports повертає 404 (роута не існує) для будь-якого автентифікованого користувача.
- [ ] Після DELETE /patients/:id у колекції reports не лишається документів з patient, що дорівнює видаленому id.
- [ ] Функція getAllReports відсутня у frontend/src; `npm run build` проходить і в backend/, і в frontend/.
- [ ] Жодних автоматичних видалень наявних сиріт — лише список для власника і видалення після явного підтвердження.
