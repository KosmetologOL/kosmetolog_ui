...
- [ ] GET /auth/refresh для користувача з active === false повертає 403 з повідомленням «Недійсний або прострочений refresh-токен»
- [ ] GET /auth/refresh для користувача з lockUntil у майбутньому повертає 403
- [ ] Для активного користувача refresh працює як раніше (200 + новий accessToken)
- [ ] Деактивований користувач у відкритій вкладці автоматично розлогінюється щонайпізніше при першому 401 після закінчення поточного access-токена (≤15 хв)
