...
- [x] Файл backend/src/scripts/cleanupProcedureRecommendations.ts відсутній у робочому дереві, згадок про нього немає ніде в коді.
- [x] Збірка бекенда проходить без помилок після видалення.
- [x] Код скрипта доступний в історії git (git log --diff-filter=D показує коміт видалення).
